...
namespace Xena.Contracts.Domain
{
    public class SubscriptionLineDto:EntityDto
    {
        ...
        public decimal PriceEach => Quantity == decimal.Zero ? decimal.Zero : (PriceNettTotal + DiscountTotal)/Quantity;
        public decimal MarginTotalRatio => PriceNettTotal == decimal.Zero ? decimal.Zero : MarginTotal / PriceNettTotal;
        public decimal MarginTotalPct => Math.Round(MarginTotalRatio * 100, 1, MidpointRounding.AwayFromZero);
        public string ArticleAbbreviation => string.IsNullOrEmpty(ArticleVariantAbbreviation) ? ArticleNumber : $"{ArticleNumber}-{ArticleVariantAbbreviation}";
    }
}